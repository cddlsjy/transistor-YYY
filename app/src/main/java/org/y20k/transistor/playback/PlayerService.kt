...
//            })
    ...
    private fun preparePlayer(playWhenReady: Boolean) {
        ...
        if (player.isPlaying) { player.stop() }
        InitializePlayerHelper().execute(playWhenReady)
    }

    /* AsyncTask to detect stream type (from v3.2.4) */
    private inner class InitializePlayerHelper : AsyncTask<Boolean, Void, Int>() {
        private var playWhenReady: Boolean = false

        override fun doInBackground(vararg params: Boolean?): Int {
            playWhenReady = params[0] ?: false
            return try {
                val connection = URL(station.getStreamUri()).openConnection()
                connection.connect()
                var contentType = connection.contentType ?: return Keys.CONNECTION_TYPE_ERROR
                if (contentType.contains(";")) {
                    contentType = contentType.substring(0, contentType.indexOf(";"))
                }
                when {
                    contentType in Keys.MIME_TYPES_HLS || contentType in Keys.MIME_TYPES_M3U -> Keys.CONNECTION_TYPE_HLS
                    contentType in Keys.MIME_TYPES_MPEG || contentType in Keys.MIME_TYPES_AAC || contentType in Keys.MIME_TYPES_OGG -> Keys.CONNECTION_TYPE_OTHER
                    else -> Keys.CONNECTION_TYPE_ERROR
                }
            } catch (e: Exception) {
                LogHelper.e(TAG, "Connection error: ${e.message}")
                Keys.CONNECTION_TYPE_ERROR
            }
        }

        override fun onPostExecute(connectionType: Int) {
            if (connectionType == Keys.CONNECTION_TYPE_ERROR) {
                Toast.makeText(this@PlayerService, getString(R.string.toastalert_unable_to_connect), Toast.LENGTH_LONG).show()
                handlePlaybackChange(PlaybackStateCompat.STATE_STOPPED)
                return
            }

            val mediaItem = MediaItem.fromUri(station.getStreamUri())
            val dataSourceFactory = DefaultHttpDataSource.Factory()
                .setUserAgent(userAgent)
                .setAllowCrossProtocolRedirects(true)

            val mediaSource = if (connectionType == Keys.CONNECTION_TYPE_HLS) {
